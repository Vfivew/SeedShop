...
import { setSeedFilters, resetSeedFilters, toggleFilterHit, toggleFilterNew, toggleFilterDiscount, setSeedProducerFilter } from '../../reducers/SeedProductReducer';

import './Filters.css';

const Filters = ({ seedFilters, dispatch, uniqueProducers, uniqueProductTypes, activeFilters }) => {
  ...
  return (
    <div>
      ...
      <div className={`filters ${showFilters ? 'show' : ''}`}>
        ...
        <button className={`reset-filters-button ${activeFilters ? 'show' : 'hidden'}`} onClick={resetFilters}>
          Очистити фільтри
        </button>
        <button className="toggle-filters-button" onClick={() => setShowFilters(!showFilters)}>
        Назад
