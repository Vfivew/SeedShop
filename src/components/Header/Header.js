...
const Header = () => {
  ...
  const handleSearch = () => {
    ...
    const foundProductType = products.find(
      product => product.productType.toLowerCase().includes(lowercaseSearchText) && lowercaseSearchText.length >= 2
    );

    if (foundProductType) {
      if (foundProductType.type === 'Овочі') {
        dispatch(setSeedFilters({ ...seedFilters, selectedProductTypes: [foundProductType.productType] }));
        navigate('/seedvegetable', { state: { productType: foundProductType.productType } });
      } else if (foundProductType.type === 'Ягоди') {
        dispatch(setSeedFilters({ ...seedFilters, selectedProductTypes: [foundProductType.productType] }));
        navigate('/seedberries', { state: { productType: foundProductType.productType } });
      }
    } else {
